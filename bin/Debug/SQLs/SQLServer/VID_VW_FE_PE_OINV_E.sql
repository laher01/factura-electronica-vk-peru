...
/****** Object:  View [dbo].[VID_VW_FE_PE_OINV_E]    Script Date: 2/7/2019 4:51:51 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[VID_VW_FE_PE_OINV_E]
...
AS
	...
	SELECT ISNULL(M0.DocCurrCod,'PEN')				[currency]
		 ...
		 --       ELSE (T0.DocTotal - T0.VatSum)
			--END										[Gravadas]
		   ,D0.SumLineTotal								[Gravadas]
	  FROM OINV T0
	  JOIN (SELECT 
				DocEntry,SUM(LineTotal) as SumLineTotal,SUM(CASE WHEN VatSumFrgn <> 0 THEN VatSumFrgn ELSE VatSum END) as IGV
			FROM
				INV1
			GROUP BY
				DocEntry) D0 ON T0.DocEntry = D0.DocEntry
