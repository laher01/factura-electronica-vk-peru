IF EXISTS(SELECT name FROM sysobjects
      WHERE name = 'VID_VW_FE_PE_GUIA_D' AND type = 'V')
   DROP VIEW VID_VW_FE_PE_GUIA_D
GO--

CREATE VIEW [dbo].[VID_VW_FE_PE_GUIA_D]
AS
	--Entrega
	SELECT  T0.DocType 'DocType',
			ISNULL(T1.ItemCode,'Servicio') 'ItemCode',
			ISNULL(LEFT(T1.Dscription,200),'Servicio') 'Descripcion',
			'NIU' 'UnidadMedida',
			T1.Price 'price',
			ISNULL((SELECT CAST(Name AS VARCHAR(30)) 
					  FROM [@FM_IVA] 
					 WHERE Code = T1.TaxCode),'10') 'exemptType',
			CASE WHEN T0.DocType = 'S' THEN 1 
				ELSE T1.Quantity 
			END Cantidad ,
			ISNULL(T0.U_BPP_MDCD,'1')  'Correlativo', 
			CAST(T1.VisOrder + 1 AS VARCHAR(30))  'LineaReferencia',
			T1.VisOrder 'LineaOrden',
			1 LineaOrden2,
			T0.DocEntry,
			T0.ObjType,
			ISNULL((SELECT SUM(CASE WHEN A0.TaxSumFrgn <> 0.0 THEN A0.TaxSumFrgn 
									ELSE A0.TaxSum 
								END) 
			...
			0.0 'Gratuitas',
			ISNULL((SELECT SUM(CASE WHEN A0.BaseSumFrg <> 0.0 THEN A0.BaseSumFrg 
									ELSE A0.BaseSum 
								END) 
					FROM DLN4 A0 JOIN [@FM_IVA] A1 ON A1.Code = A0.StaCode 
					WHERE A0.DocEntry = T0.DocEntry 
