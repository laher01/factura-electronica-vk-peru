IF EXISTS(SELECT name FROM sysobjects
      WHERE name = 'VID_FN_FE_CantidadConLetra' AND type = 'FN')
   DROP FUNCTION VID_FN_FE_CantidadConLetra
GO--

CREATE FUNCTION [dbo].[VID_FN_FE_CantidadConLetra]
(
    @Numero             Decimal(18,2),
	@CurrName			VarChar(50)
)
...
BEGIN
  ...
  BEGIN /* WHILE */
   ...
   SELECT @ImpLetra = 'SON ' + RTRIM(@lcRetorno) + ' ' + UPPER(@CurrName) +' CON ' + SUBSTRING(@sFraccion,LEN(@sFraccion)-1,2) + '/100'
