...
CREATE VIEW VID_VW_FE_PE_ORIN_R
AS

	 SELECT IFNULL(O0."U_BPP_MDTD",
				CASE O0."DocSubType"
				 WHEN '--' THEN '01'
				 WHEN 'IX' THEN '01'
				 WHEN 'IB' THEN '03'
		   ...
		   ,O0."TaxDate"			"date"
		   ,IFNULL((SELECT "Name" "Name" FROM "@FM_NOTES" WHERE "Code" = T0."U_BPP_MDTN"), CAST(O0."DocNum" AS VARCHAR(20)))	"description"
		   ,T0."DocEntry"
		   ,T0."ObjType"
     ...
     WHERE UPPER(LEFT(N0."BeginStr",1)) = 'E'
	   AND IFNULL(O0."U_BPP_MDSD",'') <> ''
	   AND IFNULL(O0."U_BPP_MDCD",'') <> ''
