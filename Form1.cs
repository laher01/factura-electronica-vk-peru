    ...
    public class TMainClassExt : TMainClass //class (TMainClass)
    {
        ...
        public override void initApp()
        {
            ...
            try
            {
                ...
                SBOApplication.StatusBar.SetText("Inicializando AddOn Factura Electronica Peru.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Warning);
                try
                {
                ...
                {
                    bCargar = false;
                    oLog.OutLog("initApp -> " + xc.Message);
                    SBOApplication.StatusBar.SetText("initApp -> " + xc.Message, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Success);
                }
                finally
                {
                ...
                if (bCargar)
                {
                    XlsFile = System.IO.Path.GetDirectoryName(TMultiFunctions.ParamStr(0)) + "\\Docs\\UDFSAPPE.xls";
                    if (!SBOFunctions.ValidEstructSHA1(XlsFile))
                    {
                        oLog.OutLog("InitApp: Estructura de datos tablas SAP - Facturación Electronica");
                        //SBOApplication.StatusBar.SetText("Inicializando AddOn Factura Electronica.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Warning);
                    ...
                    XlsFile = System.IO.Path.GetDirectoryName(TMultiFunctions.ParamStr(0)) + "\\Docs\\UDFFELECPE.xls";
                    if (!SBOFunctions.ValidEstructSHA1(XlsFile))
                    {
                        oLog.OutLog("InitApp: Estructura de datos del addon - Facturación Electronica PE");
                        //SBOApplication.StatusBar.SetText("Inicializando AddOn Factura Electronica.", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Warning);
                        if (!GlobalSettings.SBOMeta.SyncTablasUdos("1.1", XlsFile))
                        {
                            SBOFunctions.DeleteSHA1FromTable("UDFFELECPE.xls");
                            oLog.OutLog("InitApp: sincronización de Estructura de datos fallo");
                            SBOApplication.MessageBox("Estructura de datos con problemas, consulte a soporte...", 1, "Ok", "", "");
                        }
                    }
