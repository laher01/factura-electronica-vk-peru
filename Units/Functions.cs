...
namespace Factura_Electronica_VK.Functions
{
    class TFunctions
    {
        ...
        public String ValidarRuc(String Ruc)
        {
            try
            {
                if (Ruc.Trim().Length != 11)
                    return "Ruc no válido: Revise el N° de dígitos";

                if ((Ruc.Substring(0, 2) != "10") && (Ruc.Substring(0, 2) != "15") && (Ruc.Substring(0, 2) != "17") && (Ruc.Substring(0, 2) != "20"))
                    return "Ruc no válido: Revise los 2 primeros dígitos";

                var factores = "5432765432";
                var ultimoIndex = Ruc.Trim().Length - 1;
                var sumaTotal = 0;
                var residuo = 0;
                var ultimoDigitoRUC = 0;
                var ultimoDigitoCalc = 0;

                for (var i = 0; i < ultimoIndex; i++)
                {
                    sumaTotal += (int.Parse(Ruc.Trim().Substring(i, 1)) * int.Parse(factores.Substring(i, 1)));
                }
                residuo = sumaTotal % 11;

                ultimoDigitoCalc = (residuo == 10) ? 0 : ((residuo == 11) ? 1 : (11 - residuo) % 10);
                ultimoDigitoRUC = int.Parse(Ruc.Trim().Substring(ultimoIndex, 1));

                if (ultimoDigitoRUC == ultimoDigitoCalc)
                    return "OK";
                else
                    return "El RUC " + Ruc.Trim() + " no es válido";

            }
            catch (Exception e)
            {
                return "Error validar Ruc: " + e.Message;
            }
        }

        public String ValidarRUC2(String RUC)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(RUC))
                {
                    ...
                    int LongitudDocumento = RUC.Length;
                    int SumaProductos, Resultado;
                    int NroIdentificador = int.Parse(RUC.Substring(LongitudDocumento - 1, 1));

                    for (int i = 0; i < 10; i++)
                    {
                        int Valor = int.Parse(RUC.Substring(i, 1));
        ...
        public Int32 FELOGAdd(Int32 DocEntry, String ObjType, String SubType, String SeriePE, Int32 FolioNum, String Status, String sMessage, String TipoDoc, String UserCode, String JsonText, String Id, String Validation, String sDocDate, String ExternalFolio)
        {
        ...
        public Int32 FELOGUptM(Int32 DocEntry, Int32 DocEntryDoc, String ObjType, String SubType, String SeriePE, Int32 FolioNum, String Status, String sMessage, String TipoDoc, String UserCode, String JsonText, String Id, String Validation, String sDocDate, String ExternalFolio)
        {
            SAPbobsCOM.GeneralService oFELOG = null;
            SAPbobsCOM.GeneralData oFELOGData = null;
            SAPbobsCOM.GeneralDataCollection oFELOGLines = null;
        ...
        public String UpLoadDocumentByUrl(XmlDocument xmlDOC, String json, Boolean RunningUnderSQLServer, String URL, String user, String pass, String NombreArchivo) 
        {
            string postData;
        ...
        //public void UpLoadDocumentByUrl3(XmlDocument xmlDOC, String json, Boolean RunningUnderSQLServer, String URL, String user, String pass, String ExternalFolio)
        //{
        //    HttpContext context;
        ...
        public String PDFenString(String TipoDocElectAddon, String DocEntry, String ObjType, String Serie, String Folio, Boolean RunningUnderSQLServer)
        {
            ...
            String Tipo = "";
            string str1;
            SAPbobsCOM.Recordset ors = ((SAPbobsCOM.Recordset)SBO_f.Cmpny.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset));
            ...
            try
            {
                ...
                sNombreArchivo = oPath + "\\Reports\\" + TipoDocElectAddon + "_" + SBO_f.Cmpny.CompanyDB + ".rpt";
                ...
                if (Seguir)
                {
                    ...
                    try
                    {
                        ...
                        if (RunningUnderSQLServer)
                        {
                            ...
                            Pass = ((System.String)ors.Fields.Item("Pass").Value).Trim();
                            //Pass = Param.DesEncriptar(Pass);
                            connection.Password = Pass;
                            connection.UserID = ((System.String)ors.Fields.Item("Usuario").Value).Trim();
                            connection.Type = ConnectionInfoType.SQL;
                        }
                        else
                        {//DRIVER={B1CRHPROXY32};SERVERNODE=hanab1:30015;DATABASE=ZZZ_SBO_HIJUELAS
                            //DRIVER={B1CRHPROXY32};SERVERNODE=hanab1:30015;DATABASE=ZZZ_SBO_HIJUELAS
                            String strConnection = "DRIVER= {B1CRHPROXY32};UID=" + ((System.String)ors.Fields.Item("Usuario").Value).Trim(); //string de conexion contruido de las variables
                            strConnection += ";PWD=" + ((System.String)ors.Fields.Item("Pass").Value).Trim() + ";SERVERNODE=" + ((System.String)ors.Fields.Item("Servidor").Value).Trim();
                            ...
                            rpt.DataSourceConnections[0].SetConnection(((System.String)ors.Fields.Item("Servidor").Value).Trim(), SBO_f.Cmpny.CompanyDB.ToString().Trim(), ((System.String)ors.Fields.Item("Usuario").Value).Trim(), ((System.String)ors.Fields.Item("Pass").Value).Trim());
        ...
        public Boolean RescatarRPT(String TipoDocElectAddon, String sNombreArchivo)
        {
            ...
            try
            {
                ...
                if (TipoDocElectAddon == "33")
                    ...
                    Tipo = "IDN2";
                //else if (TipoDocElectAddon == "09")
                //    Tipo = "";
                //else if (TipoDocElectAddon == "12")
                //    Tipo = "";
                //else if (TipoDocElectAddon == "31")
                //    Tipo = "";
        ...
        public String Base64Encodex(String plainText)
        {
            var plainTextBytes = System.Text.Encoding.UTF8.GetBytes(plainText);
            return System.Convert.ToBase64String(plainTextBytes);
        }

        public static string Base64Decodex(string base64EncodedData)
        {
        ...
        public String CodigoHash()
        {
            try
            {
                ...
                return (BitConverter.ToString(codigoHashBytesArray, 0));
            ...
            {
                //oSBOApplication.MessageBox(e.Message + ", TRACE " + e.StackTrace);
                SBO_f.oLog.OutLog(e.Message + ", TRACE " + e.StackTrace);
                return "";
            }
        }
