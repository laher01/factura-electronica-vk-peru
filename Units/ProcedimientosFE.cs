...
namespace Factura_Electronica_VK.ProcedimientosFE
{
    class TProcedimientosFE : TvkBaseForm, IvkFormInterface
    {
        ...
        public new bool InitForm(string uid, string xmlPath, ref Application application, ref SAPbobsCOM.Company company, ref CSBOFunctions SBOFunctions, ref TGlobalVid _GlobalSettings)
        {
            ...
            try
            {
                ...
                if (GlobalSettings.RunningUnderSQLServer)
                    s = @"select DocEntry, ISNULL(U_TipoDocPE,'') TipoDocPE, ISNULL(U_ProcNomE,'') ProcNomE, ISNULL(U_ProcNomD,'') ProcNomD, ISNULL(U_ProcNomR,'') ProcNomR, ISNULL(U_ProcNomDe,'') ProcNomDe, ISNULL(U_ProcNomL,'') ProcNomL, ISNULL(U_ProcNomS,'') ProcNomS, ISNULL(U_Habili,'Y') 'Habilitada', ISNULL(U_CantLineas,0) 'CantLineas' from [@VID_FEPROCED]";
                else
                    s = @"select ""DocEntry"", IFNULL(""U_TipoDocPE"",'') ""TipoDocPE"", IFNULL(""U_ProcNomE"",'') ""ProcNomE"", IFNULL(""U_ProcNomD"",'') ""ProcNomD"", IFNULL(""U_ProcNomR"",'') ""ProcNomR"", IFNULL(""U_ProcNomDe"",'') ""ProcNomDe"", IFNULL(""U_ProcNomL"",'') ""ProcNomL"", IFNULL(""U_ProcNomS"",'') ""ProcNomS"", IFNULL(""U_Habili"",'Y') ""Habilitada"", IFNULL(""U_CantLineas"",0) ""CantLineas"" from ""@VID_FEPROCED"" ";
                ...
                s = String.Format(s, "TipoDocPE");
                oRecordSet.DoQuery(s);
                oColumn = (SAPbouiCOM.Column)(oMtx.Columns.Item("TipoDocPE"));
                FSBOf.FillComboMtx(oColumn, ref oRecordSet, false);

                ((SAPbouiCOM.Column)oMtx.Columns.Item("ProcNomL")).Visible = false;
                ((SAPbouiCOM.Column)oMtx.Columns.Item("ProcNomS")).Visible = false;

                ((SAPbouiCOM.Column)oMtx.Columns.Item("ProcNomD")).Visible = true;
                ((SAPbouiCOM.Column)oMtx.Columns.Item("ProcNomR")).Visible = true;
                ((SAPbouiCOM.Column)oMtx.Columns.Item("ProcNomDe")).Visible = false;
                ((SAPbouiCOM.Column)oMtx.Columns.Item("ProcNomE")).TitleObject.Caption = "Procedimiento";
                ((SAPbouiCOM.Column)oMtx.Columns.Item("ProcNomD")).TitleObject.Caption = "Procedimiento Detalle";
                ((SAPbouiCOM.Column)oMtx.Columns.Item("ProcNomR")).TitleObject.Caption = "Procedimiento Referencia";
                    ...
                    if ((pVal.ItemUID == "1") && ((oForm.Mode == BoFormMode.fm_ADD_MODE) || (oForm.Mode == BoFormMode.fm_UPDATE_MODE)) && (pVal.BeforeAction))
                    {
                        ...
                        if (Validar())
                            if (Limpiar())
                            {
                                ...
                                oComboBox.Select("01", BoSearchKey.psk_ByValue);
        ...
        private void GuardarRegistros()
        {
            ...
            SAPbouiCOM.EditText oEditText;
            SAPbouiCOM.ComboBox oComboBox;
            String DocEntry, ProcNomE, ProcNomD, ProcNomR, ProcNomL, ProcNomS, Hab, TipoDocPE, CantLineas, ProcNomDe;
            Int32 _return = 0;
            ...
            try
            {
                ...
                while (i <= oMtx.RowCount)
                {
                    ...
                    if (_return != 0)
                    {
                        ...
                        FSBOApp.StatusBar.SetText("Se ha guardado regsitro Tipo Documento " + TipoDocPE, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Success);
                    }
                    else
                    {
                        FSBOApp.StatusBar.SetText("No se ha guardado regsitro Tipo Documento " + TipoDocPE, BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
                    }
