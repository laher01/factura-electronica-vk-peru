IF EXISTS(SELECT name FROM sysobjects
      WHERE name = 'VID_VW_FE_PE_OINV_R' AND type = 'V')
...
AS
	...
	SELECT  '09'				[documentType]
     ...
     WHERE ISNULL(G0.U_BPP_MDSD,'') <> '' 
	   AND ISNULL(G0.U_BPP_MDCD,'') <> ''
	   AND ISNULL(T1.BaseType,'') = '15'
	...
	SELECT '91'				[documentType]
		   ,ISNULL(O0.NumAtCard,'--')	[referencedFolio]
		   ,O0.TaxDate			[date]
     ...
     WHERE ISNULL(T0.FolioNum,0) <> 0
	   AND ISNULL(G1.BaseType,'') = '17'
	   AND ISNULL(O0.NumAtCard,'--') <> ''
